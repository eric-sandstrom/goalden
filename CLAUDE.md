...
`firebase-functions` v7 (v2 API). Region `europe-west1`, `maxInstances: 10` — set globally in `functions/src/index.ts` via `setGlobalOptions`. Each function is re-exported from `index.ts` and lives in its own `functions/src/*.ts` module. The list below is the live set (see `index.ts` for the source of truth).

**Scheduled (`onSchedule`)**

| Function | Job |
|---|---|
| `pollFootballData` | Polls football-data.org, upserts `fixtures/{matchId}` |
| `pollStandings` | Polls group standings |
| `pollTeams` | Polls team metadata (names, flags) |
| `rebuildLeaderboard` | Rebuilds the denormalised leaderboard / totals rollup |

**Firestore triggers**

| Function | Trigger | Job |
|---|---|---|
| `scoreMatch` | `onDocumentUpdated('fixtures/{matchId}')` → `FINISHED` | Scores predictions for the match; updates `predictions/{uid}/matches/{matchId}.points` + `users/{uid}/totals` |
| `onUserProfileChange` | `onDocumentWritten('users/{uid}')` | Keeps the leaderboard rollup in sync with profile changes |
| `autoEnrollOnUserCreate` | `onDocumentCreated('users/{uid}')` | Auto-enrols a new user into the global league |

**Callable (`onCall`)**

| Domain | Functions |
|---|---|
| Leagues | `createLeague`, `joinLeague`, `leaveLeague`, `deleteLeague`, `regenerateInviteCode`, `transferOwnership`, `kickMember` |
| Global leagues | `createGlobalLeague`, `syncGlobalLeague`, `deleteGlobalLeague` |
| Competitions (multi-comp) | `syncCompetitionsFromApi`, `setCompetitionActive`, `migrateToMultiComp` |
| Admin | `grantAdminRole`, `revokeAdminRole`, `correctFixtureScore`, `getAdminMetrics` |
| AI (Gemini) | `generatePredictorPersonality`, `generateTheme` |
| Notifications | `sendTestNotification`, `broadcastNotification` |

**Dev-only callables** (local/testing helpers): `devRebuildLeaderboardNow`, `devSeedFakeUsers`, `devClearFakeUsers`, `devFinishMatch`, `devSetFixtureState`, `devSetKickoffTime`, `devResetMyState`, `devClearMyPersonality`, `devSignInAsAdmin`, `devPollTeamsNow`, `devPollFixturesNow`, `devPollLiveDetailNow`, `devPollStandingsNow`.

> **Divergences from the original v1 spec:**
> - Invite codes are generated by a helper (`functions/src/lib/invite-code.ts`) called inside `createLeague` / `regenerateInviteCode` — there is **no** standalone `generateInviteCode` function.
> - `scorePodium`, `scoreBracketRound`, and `sendMatchReminder` (named in earlier drafts) are **not implemented**. Podium/bracket scoring and the 1-hour "predict now" reminder are still TODO; notifications are currently manual (`sendTestNotification` / `broadcastNotification`).
> - The backend has grown beyond the original scope: multi-competition support, global leagues, AI features (predictor personality + theme generation via Gemini, `GEMINI_API_KEY` secret), and admin roles/tools.
> - Local emulator dev for functions: see the workflow in the README / memory — functions are TS compiled to `functions/lib/`; use `npm run emulators:watch` from the **main** checkout (worktrees can't run their own emulator).
